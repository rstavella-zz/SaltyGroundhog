...
<body>
		<div id="header">
			<center><h1>Edit Life Events</h1></center>
		</div>
		   <div class="main">
            ...
            $sql = "SELECT c.cust_id, c.first_name, c.last_name FROM customers as c, life_events as le, clientprofessional as cp WHERE cp.prof_id = '" . $_SESSION['prof_id'] . "' AND cp.cust_id = c.cust_id AND le.cust_id = c.cust_id AND le.le_id = '" . $le_id . "'";
            $result = pg_query($sql);
            $row = pg_fetch_array($result);
            $name = $row["first_name"] . $row["last_name"];
            $cust_id = $row["cust_id"];
            echo "Set As Life Event For <select name='cust_id' id='cust_id'>";
          ...
          <?php
            ...
            $row = pg_fetch_array($result);
          ?>
          <li class="text-info">Change Name</li>
          <li><input type="text" value="<?php echo $row['event_name']?>" name="event_name" placeholder="Event Name" required></li>
          <div class="clear"></div>    
          <br>
          <?php
          ...
          <li><input type="text" value="<?php echo $row['location']?>" name="location" placeholder="Location"></li>
          <div class="clear"></div>    
          </ul>
          <ul>
          <?php
          ...
          <li><input type="text" name="event_description" value="<?php echo $row['event_description'] ?>" placeholder="Description"></li>
          <div class="clear"></div>    
          </ul>
          <br>
          ...
          <br>
          <br>
          <input type="submit" name="delete_life_event" value="Delete Life Event" class="btn btn-primary"></center>
          <br>
          <br>
            </form>
        </div>
 ...
 <?php
     if(isset($_POST['submit_changes'])){
        ...
        $query = pg_query("UPDATE life_events SET cust_id = '" . $cust_id . "', event_name = '" . $event_name . "', event_description = '" . $event_description . "', date = '" . $date . "', location = '" . $location . "' WHERE le_id = '" . $le_id . "'");
        if (!$query) {
            ...
            echo "alert('$message');";
            echo "window.location.href = 'viewLifeEvent.php?id=" . $le_id . "';";
            echo "</script>";
        }
     }

      if(isset($_POST['delete_life_event'])){
      $query = pg_query("DELETE FROM life_events WHERE le_id = '" . $le_id . "'");
        if (!$query) {
             $errormessage = pg_last_error();
             $message = "Error with entry. Please check fields.";
             echo "<script type='text/javascript'>alert('$message');</script>";
            ...
            $message = "Successfully Deleted Life Event";
            echo "<script type='text/javascript'>";
            echo "alert('$message');";
