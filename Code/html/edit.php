...
Senior Project
True Course Website
This page is to allow a professional to edit a clients profile
-->
...
<?php
   ...
   $query = pg_query("SELECT cu.Cust_ID, cu.First_Name, cu.Last_Name, cu.Street_Address, cu.active_status, cu.Zipcode, cu.state, cu.city, cu.country,
                             cu.home_phone, cu.work_phone, cu.cell_phone, cu.gender, cu.martital_status, cu.email, cu.dob
                      FROM customers as cu
                      WHERE cu.Cust_ID = '$identity'");

   $query1 = pg_query("SELECT cu.Cust_ID, cb.fav_food, cb.fav_book, cb.pref_call_time, cb.self_awareness_practice  FROM customers as cu, customer_bios as cb WHERE cu.Cust_ID = '$identity' and cu.Cust_ID = cb.Cust_ID");

   $results=pg_fetch_array($query);
   $results1=pg_fetch_array($query1);

   #This is break the birthday down from the database so that it can be edited and resubmitted into the database
   $birthDay = $results['dob'];                 
   $explodeBirthDay = explode('-',$birthDay);
   $dobMonth = $explodeBirthDay['0'];
   $dobDay = $explodeBirthDay['1'];
   $dobYear = $explodeBirthDay['2'];

?>
<!-- Displays the edit page for a clients profile. It auto populates from the data in the database -->
...
<div class="main">
   <div class="main-section agile">
      <div class="login-form">
         <form method="post" action="edit.php?id=<?php echo $identity ?>">
            <ul>
      ...
      <ul>
		 ...
		 <li><input type="text" name="email" value="<?php echo $results['email'];?>"></li>
		 <div class="clear"></div>
      </ul>
      <ul>
      ...
      <ul>
		 ...
		 <li class="se"><select class="form-dropdown" id="martital_status" name="martital_status">
				<option value="<?php echo $results['martital_status'];?>" selected="selected"><?php echo $results['martital_status'];?></option>
      ...
      <ul>
		  ...
		  <select class="form-dropdown" id="pref_call_time" name="pref_call_time">
				<option value="<?php echo $results1['pref_call_time'];?>" selected="selected"><?php echo $results1['pref_call_time'];?></option>
     ...
     <ul>
		...
		<li><input type="text" name="self_awareness_practice" value="<?php echo $results1['self_awareness_practice'];?>" </li>
		<div class="clear"></div>
     </ul>
     <ul>
		 <li class="text-info">Favorite Food </li>
		 <li><input type="text" name="fav_food" value="<?php echo $results1['fav_food'];?>"></li>
		 <div class="clear"></div>
     </ul>
     <ul>
		 <li class="text-info">Favorite Book </li>
		 <li><input type="text" name="fav_book" value="<?php echo $results1['fav_book'];?>"></li>
		 <div class="clear"></div>
     </ul>
     <ul>
		  <li class="text-info">Active Status </li> <li class="se">
		  <select class="form-dropdown" id="active_status" name="active_status">
				<option value="<?php echo $results['active_status'];?>" selected="selected"><?php echo $results['active_status'];?></option>
     ...
     <ul>
		<li>
			<input type="button" class="btn btn-primary" onclick="location.href='hobbyPage.php?id=<?php echo $identity ?>';" value="Add Hobby" />
			<input type="button" class="btn btn-primary" onclick="location.href='jobPage.php?id=<?php echo $identity ?>';" value="Add Job" />
			<input type="button" class="btn btn-primary" onclick="location.href='educationPage.php?id=<?php echo $identity ?>';" value="Add Education" />
		</li>
		<div class="clear"></div>
     </ul> 
     <ul>
...
<?php
	...
	$martital_status=$_POST['martital_status'];
	$email=$_POST['email'];
	$gender=$_POST['gender'];
	$pref_call_time=$_POST['pref_call_time'];
	$self_awareness_practice=$_POST['self_awareness_practice'];
	...
	$email = preg_replace("/[^@.a-zA-Z0-9\s]/", "", $email);
        ...
        $query1=pg_query("update customers set first_name='$first_name', last_name='$last_name', active_status='$active_status', street_address='$street_address', city='$city', state='$state', country='$country', home_phone='$home_phone', work_phone='$work_phone', cell_phone='$cell_phone',  gender='$gender', martital_status='$martital_status', dob='$dob', email='$email'  where Cust_ID='$identity'");
        $query2=pg_query("update customer_bios set fav_food='$fav_food', fav_book='$fav_book', pref_call_time='$pref_call_time',
                                  self_awareness_practice='$self_awareness_practice' where cust_id='$identity'");
